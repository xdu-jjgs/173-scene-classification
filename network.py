...
class AttentionCropFunction(torch.autograd.Function):
    ...
    def forward(self, images, locs):
        ...
        x = torch.stack([unit.t()] * 18).type(torch.float32)  # 改变通道数3==》18
        y = torch.stack([unit] * 18).type(torch.float32)  # 改变通道数3==》18
        ...
        for i in range(images.size(0)):  # 表示每一个batch中的图像
            ...
            # TODO 这里的size记得改
            xamp = F.upsample(before_upsample, size=(32, 32), mode='bilinear', align_corners=True)
...
class CNNCell(nn.Module):
    ...
    def __init__(self, input_bands, num_classification):
        ...
        '''
        # self.conv0 = nn.Conv2d(input_bands, 16, 3, 2, 0)  # (indim,outdim,filter,stride,pad)
        self.conv0 = nn.Conv2d(input_bands, 16, 3, 2, 1)
        self.bn0 = nn.BatchNorm2d(16)  # 16*16

        # self.conv1 = nn.Conv2d(16, 32, 3, 2, 0)  # (indim,outdim,filter,stride,pad)
        self.conv1 = nn.Conv2d(16, 32, 3, 2, 1)
        self.bn1 = nn.BatchNorm2d(32)  # 8*8

        self.conv2 = nn.Conv2d(32, 64, 3, 2, 1)
        self.bn2 = nn.BatchNorm2d(64)  # 4*4

        # self.conv3 = nn.Conv2d(64, 128, 3, 2, 0)
        self.conv3 = nn.Conv2d(64, 128, 3, 2, 1)
        self.bn3 = nn.BatchNorm2d(128)  # 2*2

        # self.conv4 = nn.Conv2d(128, 256, 3, 1, 0)
        self.conv4 = nn.Conv2d(128, 256, 3, 2, 1)
        self.bn4 = nn.BatchNorm2d(256)  # 1*1
    ...
    def forward(self, x):
        ...
        box = self.apn1(feature.reshape(-1, 32 * 8 * 8))
...
class discriminator(nn.Module):
    def __init__(self, num_classification):
        ...
        self.num_classification = num_classification
        # TODO 这里参数记得改
        self.mainDNet = RAM_DIFF(input_bands=18, input_zise=32, num_classification=num_classification, step=3,
                                 effective_step=[0, 1, 2])
