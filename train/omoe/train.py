...
def worker(rank_gpu, args):
    ...
    # build model
    omoe = build_model(NUM_CHANNELS, NUM_CLASSES)
    omoe.to(device)
    # print(model)
    # build criterion
    ...
    # build optimizer
    optimizer = build_optimizer(omoe)
    # build scheduler
    scheduler = build_scheduler(optimizer)

    # mixed precision
    omoe, optimizer = amp.initialize(omoe, optimizer, opt_level=args.opt_level)
    # DDP
    omoe = DistributedDataParallel(omoe)
    ...
    if args.checkpoint is not None:
        ...
        omoe.load_state_dict(checkpoint['model']['state_dict'])
    ...
    while True:
        ...
        # train
        omoe.train()  # set model to training mode
        metric.reset()  # reset metric
        train_bar = tqdm(train_dataloader, desc='training', ascii=True)
        train_loss = 0.
        weights_epoch = np.zeros((len(omoe.module.experts)))
        ...
        if args.no_validate:
            continue
        omoe.eval()  # set model to evaluation mode
        metric.reset()  # reset metric
