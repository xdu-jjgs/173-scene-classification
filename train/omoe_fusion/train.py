...
def worker(rank_gpu, args):
    ...
    # build model
    omoe_fusion = build_model(NUM_CHANNELS, NUM_CLASSES)
    omoe_fusion.to(device)
    # print(model)
    # build criterion
    ...
    # build optimizer
    optimizer = build_optimizer(omoe_fusion)
    # build scheduler
    scheduler = build_scheduler(optimizer)

    # mixed precision
    omoe_fusion, optimizer = amp.initialize(omoe_fusion, optimizer, opt_level=args.opt_level)
    # DDP
    omoe_fusion = DistributedDataParallel(omoe_fusion)
    ...
    if args.checkpoint is not None:
        ...
        omoe_fusion.load_state_dict(checkpoint['model']['state_dict'])
    ...
    while True:
        ...
        weights_epoch /= len(train_dataloader)
        ...
        if dist.get_rank() == 0:
            ...
            for ind in range(len(experts)):
                writer.add_scalar('train/weight_expert_{}'.format(ind + 1), weights_epoch[ind], epoch)
        logging.info(
        ...
        if args.no_validate:
            continue
        omoe_fusion.eval()  # set model to evaluation mode
        metric.reset()  # reset metric
        ...
        weights_epoch /= len(val_dataloader)
        PA, mPA, Ps, Rs, F1S = metric.PA(), metric.mPA(), metric.Ps(), metric.Rs(), metric.F1s()
        if dist.get_rank() == 0:
            ...
            for ind in range(len(experts)):
                writer.add_scalar('val/weight_expert_{}'.format(ind + 1), weights_epoch[ind], epoch)
        if PA > best_PA:
            best_epoch = epoch
        ...
        if dist.get_rank() == 0:
            checkpoint = {
                'model': {
                    'state_dict': omoe_fusion.state_dict(),
                },
                'optimizer': {
