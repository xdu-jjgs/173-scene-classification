...
from network_vnr_gf import discriminator
...
class TrainDataset(Dataset):
    def __init__(self):
        ...
        # self.mode = mode
        self.sar, self.msi, self.label, = self.load_data()
        self.class_num = 7
        self.label = one_hot(self.label, self.class_num)

    def load_data(self):
        data = sio.loadmat('data/vnr-gf/dataset_vnr-fg.mat')
        vnr = data['vnr']  # N*32*32*8
        gf = data['gf']  # N*32*32*10
        label = data['label']  # N
...
def main():
    ...
    parser.add_argument('--num_class', type=int, default=7, help='class num in dataset')
    parser.add_argument('--loss_threshold', type=int, default=0.02, help='class num in dataset')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning Rate. Default=0.001')
    parser.add_argument('--batchSize', type=int, default=512, help='training batch size')  # 32768/1024=32iter

    parser.add_argument('--snapshots', type=int, default=10, help='save the checkpoint every snapshot')
    parser.add_argument('--checkpoint', type=str, default='RCNN_epoch_49.pth', help='sr pretrained base model')
    parser.add_argument('--pretrained', type=bool, default=False)
    parser.add_argument('--save_folder', type=str, default='checkpoint/vnr_exp0/', help='Location to save checkpoint models')
